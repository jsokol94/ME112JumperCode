...
Functions all use Numpy for Matlab-like syntax so
they are easy to translate to Matlab. Points are 2 element arrays (x,y).
Trajectories are Nx2 arrays with points in each row, angles in radians.
"""
...
#########################
# Return True/False if linkage satisfiess Grashof criteroun.
...
#########################
# Given a sequence of points (x,y), return array of links between them.
# The Last link closes a loop from the last point back to the first.
def joints2links(jointarray):
    Npoints = np.shape(jointarray)[0]   #How many rows in array?
    linkarray = np.zeros(Npoints,float) #array for output
    
    for i in range(0,Npoints-1):
        dx = jointarray[i+1,0]-jointarray[i,0]
        dy = jointarray[i+1,1]-jointarray[i,1]
        linkarray[i]=np.sqrt(dx*dx+dy*dy)
    
    dx = jointarray[0,0]-jointarray[Npoints-1,0] #last link (close loop)
    dy = jointarray[0,1]-jointarray[Npoints-1,1]
    linkarray[Npoints-1]=np.sqrt(dx*dx+dy*dy)
